...
const GOLDEN: Array<[input: { source: string; severity: string; message: string }, expected: string]> = [
  [{ source: 'review', severity: 'major', message: 'no test for the new branch' }, 'missing_test'],
  [{ source: 'review', severity: 'minor', message: 'unrelated refactor included' }, 'scope_creep'],
  [{ source: 'review', severity: 'major', message: 'TODO: implement error case' }, 'incomplete_impl'],
  [{ source: 'review', severity: 'minor', message: 'use camelCase here' }, 'style_lint'],
  [{ source: 'review', severity: 'critical', message: 'sql injection in user input' }, 'security'],
  [{ source: 'review', severity: 'major', message: 'O(n^2) loop on hot path' }, 'performance'],
  [{ source: 'review', severity: 'minor', message: 'consider extracting helper' }, 'maintainability'],
  [{ source: 'review', severity: 'minor', message: 'README not updated' }, 'doc_gap'],
  [{ source: 'review', severity: 'minor', message: 'something completely surprising' }, 'other'],

  // Negative goldens — must NOT trigger an over-eager pattern.
  [{ source: 'review', severity: 'minor', message: 'there is no known issue here' }, 'other'],         // not 'missing_test' (precedence guard)
  [{ source: 'review', severity: 'minor', message: 'lifestyle of the codebase' }, 'other'],            // word-boundary `\bstyle\b` correctly excludes substrings of 'lifestyle' — guard against future removal of `\b`
  [{ source: 'review',    severity: 'minor', message: 'documenting that we accepted this scope' }, 'doc_gap'], // 'docs' wins legitimately
];
...
describe('concern-classifier', () => {
  ...
  it('never reads back the raw message past classification', () => {
    const out = classifyConcern({ source: 'review', severity: 'major', message: 'super secret path /Users/x/y' });
...
describe('concern classifier (audit-domain extension)', () => {
  it('produces ≥3 distinct non-other categories from the round-1 fixtures', () => {
    const cats = new Set(AUDIT_FIXTURES.map(f => classifyConcern({ source: 'review', severity: 'medium', message: f.message })));
    cats.delete('other');
  ...
  for (const fixture of AUDIT_FIXTURES) {
    it(`classifies "${fixture.message.slice(0, 60)}…" as not 'other'`, () => {
      const cat = classifyConcern({ source: 'review', severity: 'medium', message: fixture.message });
      expect(cat).not.toBe('other');
    });
  }

  it('source-code review patterns still classify correctly', () => {
    expect(classifyConcern({ source: 'review', severity: 'medium', message: 'missing unit tests for X' })).toBe('missing_test');
    expect(classifyConcern({ source: 'review', severity: 'critical', message: 'sql injection in handler' })).toBe('security');
  });
});
