...
import { runTasks } from '@zhixuan92/multi-model-agent-core/lifecycle/dispatch-task';
...
const factories: Record<string, EventFactory> = {
  ...
  stage_change: () => ({ event: 'stage_change', ts: TS, batchId: BATCH_ID, taskIndex: 0, from: 'implementing', to: 'spec_review', attempt: 1, attemptCap: 3, implTier: 'standard', reviewerTier: 'standard', escalated: false }),
  ...
  verify_skipped: () => ({ event: 'verify_skipped', ts: TS, batchId: BATCH_ID, taskIndex: 0, reason: 'no_command', stage: 'verifying' }),
