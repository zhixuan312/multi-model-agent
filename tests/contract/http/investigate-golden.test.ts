...
describe('contract: POST /investigate golden', () => {
  for (const stage of STAGES) {
    it(`produces the ${stage} envelope`, async () => {
      const h = await boot({ provider: mockProvider({ stage }), cwd: process.cwd() });
      try {
        const dispatch = await fetch(`${h.baseUrl}/investigate?cwd=${encodeURIComponent(process.cwd())}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${h.token}` },
          body: JSON.stringify({ question: 'golden investigate test prompt — echo findings' }),
        });
        ...
        const goldenRel = `../goldens/endpoints/investigate-${stage}.json`;
