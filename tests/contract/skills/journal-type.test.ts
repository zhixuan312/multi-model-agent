...
describe('contract: journal type vocabulary', () => {
  ...
  // journal type vocabulary + topic/index format against the COMMITTED skill files and goal
  // conditions below, which are the surfaces that actually ship. (Each assertion previously made
  // against schema.md is already duplicated against implement.md, review.md, SKILL.md, or the
  // goal conditions.)
  ...
  const goalConditionsPath = resolve('packages/server/src/application/goal-conditions.ts');
  ...
  const goalConditions = readFileSync(goalConditionsPath, 'utf8');
  ...
  it('goal condition references type classification', () => {
    expect(goalConditions).toContain('type (decision/design/behavior/process/knowledge/style)');
  });
