...
found — 1 critical finding`);
    ...
    const task: TaskSpec = {
      prompt: 'audit this code', parallelTarget: 'audit this code', cwd: os.tmpdir(),
...
found — security issue found`);
    ...
    const task: TaskSpec = {
      prompt: 'audit security', parallelTarget: 'audit security', cwd: os.tmpdir(),
      reviewPolicy: 'none', timeoutMs: 60_000, tools: 'full',
    };
...
found — style issue`);
    ...
    const task: TaskSpec = {
      prompt: 'audit code style', parallelTarget: 'audit code style', cwd: os.tmpdir(),
      reviewPolicy: 'none', timeoutMs: 60_000, tools: 'full',
    };
