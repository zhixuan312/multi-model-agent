...
import { createHttpServerLog, JsonlWriter, JsonlWriterOptions } from '../../packages/core/src/index.js';

function captureLines(): { logger: ReturnType<typeof createHttpServerLog>; lines: string[] } {
  const lines: string[] = [];
  const writer = new JsonlWriter({
    dir: '/tmp/test-http-events',
    openSync: () => 1,
    closeSync: () => {},
    writeSync: (_fd: number, data: string) => { lines.push(data.trimEnd()); },
    mkdirSync: () => {},
  } as JsonlWriterOptions);
...
describe('§5.9 canonical HTTP diagnostic events', () => {
  ...
  it('logger does not produce api_request_* events from project-lifecycle calls (poll-spam guard)', () => {
    ...
    for (let i = 0; i < 10; i++) {
      // No logger calls for batch polling
    }
    const names = eventNames(lines);
