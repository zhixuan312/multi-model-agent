...
describe('CanonicalUsage', () => {
  it('empty accumulator has zero numbers and null gap fields', () => {
    ...
    expect(u.cachedCreationTokens).toBeNull();
  ...
  it('mergeUsage promotes null + number to number (treats null as 0 ONLY for accumulation, not for absence-signaling)', () => {
    const u = mergeUsage(makeEmptyUsage(), { inputTokens: 100, outputTokens: 50, cachedReadTokens: 10, cachedCreationTokens: 0, reasoningTokens: null });
    expect(u.cachedReadTokens).toBe(10);
    expect(u.cachedCreationTokens).toBe(0);
    expect(u.reasoningTokens).toBeNull();
  });

  it('mergeUsage with all-null source preserves null', () => {
    const u = mergeUsage(makeEmptyUsage(), { inputTokens: 100, outputTokens: 50, cachedReadTokens: null, cachedCreationTokens: null, reasoningTokens: null });
    expect(u.cachedReadTokens).toBeNull();
    expect(u.cachedCreationTokens).toBeNull();
    expect(u.reasoningTokens).toBeNull();
  });
  ...
  it('mergeUsage accumulates numbers across multiple turns', () => {
    const a = mergeUsage(makeEmptyUsage(), { inputTokens: 100, outputTokens: 50, cachedReadTokens: 10, cachedCreationTokens: 0, reasoningTokens: 5 });
    const b = mergeUsage(a, { inputTokens: 200, outputTokens: 100, cachedReadTokens: 20, cachedCreationTokens: 0, reasoningTokens: 10 });
    expect(b.inputTokens).toBe(300);
    expect(b.outputTokens).toBe(150);
    expect(b.cachedReadTokens).toBe(30);
    expect(b.cachedCreationTokens).toBe(0);
    expect(b.reasoningTokens).toBe(15);
  });

  it('mergeUsage switches from null to number once a real value appears', () => {
    const a = mergeUsage(makeEmptyUsage(), { inputTokens: 100, outputTokens: 50, cachedReadTokens: null, cachedCreationTokens: null, reasoningTokens: null });
    expect(a.cachedReadTokens).toBeNull();
    expect(a.cachedCreationTokens).toBeNull();
    expect(a.reasoningTokens).toBeNull();
    const b = mergeUsage(a, { inputTokens: 50, outputTokens: 25, cachedReadTokens: 5, cachedCreationTokens: 0, reasoningTokens: 2 });
  ...
  it('zero is a real observed value, distinct from null', () => {
    const u = mergeUsage(makeEmptyUsage(), { inputTokens: 100, outputTokens: 50, cachedReadTokens: 0, cachedCreationTokens: 0, reasoningTokens: 0 });
  ...
  it('null + null stays null even after multiple turns', () => {
    ...
    u = mergeUsage(u, { inputTokens: 100, outputTokens: 50, cachedReadTokens: null, cachedCreationTokens: null, reasoningTokens: null });
    u = mergeUsage(u, { inputTokens: 100, outputTokens: 50, cachedReadTokens: null, cachedCreationTokens: null, reasoningTokens: null });
    expect(u.cachedReadTokens).toBeNull();
    expect(u.cachedCreationTokens).toBeNull();
    expect(u.reasoningTokens).toBeNull();
  ...
  it('zero + zero accumulates to zero', () => {
    let u = makeEmptyUsage();
    u = mergeUsage(u, { inputTokens: 100, outputTokens: 50, cachedReadTokens: 0, cachedCreationTokens: 0, reasoningTokens: 0 });
    u = mergeUsage(u, { inputTokens: 100, outputTokens: 50, cachedReadTokens: 0, cachedCreationTokens: 0, reasoningTokens: 0 });
    expect(u.cachedReadTokens).toBe(0);
    expect(u.cachedCreationTokens).toBe(0);
    expect(u.reasoningTokens).toBe(0);
  ...
  it('number + null after a prior real value preserves the accumulated number', () => {
    let u = makeEmptyUsage();
    u = mergeUsage(u, { inputTokens: 100, outputTokens: 50, cachedReadTokens: 10, cachedCreationTokens: 0, reasoningTokens: 5 });
    // Next turn doesn't expose cached/reasoning (null) — should preserve accumulated values
    u = mergeUsage(u, { inputTokens: 50, outputTokens: 25, cachedReadTokens: null, cachedCreationTokens: null, reasoningTokens: null });
    expect(u.cachedReadTokens).toBe(10);
    expect(u.cachedCreationTokens).toBe(0);
  ...
  it('mixed turns accumulate correctly: null then number then null', () => {
    ...
    u = mergeUsage(u, { inputTokens: 200, outputTokens: 100, cachedReadTokens: 15, cachedCreationTokens: 0, reasoningTokens: 8 });
  ...
  it('input and output tokens accumulate independently of null gap fields', () => {
    let u = makeEmptyUsage();
    u = mergeUsage(u, { inputTokens: 100, outputTokens: 50, cachedReadTokens: null, cachedCreationTokens: null, reasoningTokens: null });
    u = mergeUsage(u, { inputTokens: 200, outputTokens: 100, cachedReadTokens: 10, cachedCreationTokens: 0, reasoningTokens: null });
  ...
  it('mergeUsage returns a new object', () => {
    ...
    const result = mergeUsage(acc, { inputTokens: 100, outputTokens: 50, cachedReadTokens: 10, cachedCreationTokens: 0, reasoningTokens: 5 });
    expect(result).not.toBe(acc);
  });

  it('mergeUsage does not mutate the accumulator', () => {
    const acc = makeEmptyUsage();
    mergeUsage(acc, { inputTokens: 100, outputTokens: 50, cachedReadTokens: 10, cachedCreationTokens: 0, reasoningTokens: 5 });
    expect(acc.inputTokens).toBe(0);
    expect(acc.outputTokens).toBe(0);
    expect(acc.cachedReadTokens).toBeNull();
    expect(acc.cachedCreationTokens).toBeNull();
  ...
  it('handles large token counts', () => {
    let u = makeEmptyUsage();
    u = mergeUsage(u, { inputTokens: 1_000_000, outputTokens: 500_000, cachedReadTokens: 100_000, cachedCreationTokens: 0, reasoningTokens: 50_000 });
    u = mergeUsage(u, { inputTokens: 2_000_000, outputTokens: 1_000_000, cachedReadTokens: 200_000, cachedCreationTokens: 0, reasoningTokens: 100_000 });
