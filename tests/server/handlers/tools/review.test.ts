// tests/server/handlers/tools/review.test.ts
...
function makeTmpCwd(): string {
  return mkdtempSync(join(tmpdir(), 'mmagent-review-test-'));
}
...
describe('POST /review handler', () => {
  ...
  it('returns 202 with empty body (all fields are optional for review)', async () => {
    const s = await startTestServerWithAgents();
    const cwd = makeTmpCwd();
    try {
      const res = await fetch(`${s.url}/review?cwd=${encodeURIComponent(cwd)}`, {
      ...
      // review.inputSchema has all-optional fields; empty body is valid
      expect(res.status).toBe(202);
    } finally {
      await s.stop();
    }
  ...
  it('returns 400 invalid_request when focus contains an invalid value', async () => {
    const s = await startTestServerWithAgents();
    const cwd = makeTmpCwd();
    try {
      const res = await fetch(`${s.url}/review?cwd=${encodeURIComponent(cwd)}`, {
        ...
        body: JSON.stringify({ focus: ['not-a-valid-focus'] }),
      });
