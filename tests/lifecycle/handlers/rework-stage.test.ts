...
function makeState(send: ReturnType<typeof vi.fn>, openSessionSpy: ReturnType<typeof vi.fn>) {
  ...
  return {
    ...
    state: {
      ...
      gates: { review: { outcome: 'advance', payload: { verdict: 'changes_required', findings: [{ source: 'quality', text: 'concern_one_sentinel_88' }] } } },
...
describe('reworkHandler — progress watchdog wiring (smoke)', () => {
  it('runs to completion with watchdog wired and reviewVerdict approved-by-default', async () => {
    ...
    const state: any = {
      ...
      gates: { review: { outcome: 'advance', payload: { verdict: 'changes_required', findings: [{ source: 'quality', text: 'concern' }] } } },
