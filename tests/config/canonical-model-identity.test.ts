...
describe('canonicalIdentity', () => {
  it('strips embedded credentials from endpoint', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://user:pass@api.example.com/v1', model: 'gpt-x' });
    expect(id.normalizedEndpoint).not.toContain('user');
    expect(id.normalizedEndpoint).not.toContain('pass');
  });

  it('strips default :443 port', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://api.example.com:443/v1', model: 'gpt-x' });
    expect(id.normalizedEndpoint).not.toContain(':443');
  });

  it('strips trailing slash from path endpoint', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://api.example.com/v1/', model: 'gpt-x' });
    expect(id.normalizedEndpoint).toBe('https://api.example.com/v1');
  });

  it('strips trailing slash from root endpoint', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://api.example.com/', model: 'gpt-x' });
  ...
  it('strips query strings', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://api.example.com/v1?key=secret', model: 'gpt-x' });
    expect(id.normalizedEndpoint).not.toContain('secret');
    expect(id.normalizedEndpoint).not.toContain('?');
  });

  it('strips hash fragments', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://api.example.com/v1#section', model: 'gpt-x' });
    expect(id.normalizedEndpoint).not.toContain('#');
    expect(id.normalizedEndpoint).toBe('https://api.example.com/v1');
  });

  it('strips default :80 port for http', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'http://api.example.com:80/v1', model: 'gpt-x' });
  ...
  it('sanitizes malformed endpoint with credentials and query in fallback', () => {
    const id = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'api.example.com/v1?key=secret', model: 'gpt-x' });
    expect(id.normalizedEndpoint).not.toContain('secret');
    expect(id.normalizedEndpoint).not.toContain('?');
  });
  ...
  it('identity equality is structural', () => {
    const a = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://x/v1', model: 'gpt' });
    const b = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://x/v1/', model: 'gpt' });
    expect(identityEquals(a, b)).toBe(true);
  });

  it('different model yields different identity', () => {
    const a = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://x/v1', model: 'gpt' });
    const b = canonicalIdentity({ type: 'openai-compatible', baseUrl: 'https://x/v1', model: 'haiku' });
