...
describe('Lifecycle gates', () => {
  ...
  it('scopes manual assertions and voids them after both supported exits from satisfied', () => {
    ...
    expect(evaluateLifecycleGate({ contract, phase: 'discover', requirements: [], acceptanceCriteria: [], decisions: [], events: [satisfied] })).toMatchObject({ status: 'green' });
    const reopened = event('phase_reopened', { phase: 'discover', previous_state: 'satisfied', new_state: 'reopened', reason: 'finding' }, 2);
    expect(evaluateLifecycleGate({ contract, phase: 'discover', requirements: [], acceptanceCriteria: [], decisions: [], events: [satisfied, reopened] })).toMatchObject({ status: 'red' });
    const entered = event('phase_entered', { phase: 'discover', previous_state: 'satisfied', new_state: 'active' }, 2);
    expect(evaluateLifecycleGate({ contract, phase: 'discover', requirements: [], acceptanceCriteria: [], decisions: [], events: [satisfied, entered] })).toMatchObject({ status: 'red' });
    ...
    expect(evaluateLifecycleGate({ contract, phase: 'discover', requirements: [], acceptanceCriteria: [], decisions: [], events: [otherContract, contractChange] })).toMatchObject({ status: 'red' });
  });
});