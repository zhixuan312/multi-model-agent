...
describe('HeartbeatTimer.getHeadlineSnapshot', () => {
  ...
  it('omits saved-cost clause when savedCostUSD is zero', () => {
    const ht = new HeartbeatTimer(() => {}, { provider: 'gpt-5', parentModel: 'claude-opus-4-7' });
    ht.start(5);
    ht.applyCost({ costUSD: 0.01, savedCostUSD: 0 });
    const snap = ht.getHeadlineSnapshot();
    expect(snap.statsClause).toBe(''); // counters all zero, savedCost zero
  });

  it('emits saved-cost clause with multiplier when both costUSD and savedCostUSD are positive finite', () => {
    const ht = new HeartbeatTimer(() => {}, { provider: 'gpt-5', parentModel: 'claude-opus-4-7' });
    ht.start(5);
    ht.applyCost({ costUSD: 0.01, savedCostUSD: 0.10 });
    const snap = ht.getHeadlineSnapshot();
  ...
  it('omits multiplier when costUSD is zero', () => {
    const ht = new HeartbeatTimer(() => {}, { provider: 'gpt-5', parentModel: 'claude-opus-4-7' });
    ht.start(5);
    ht.applyCost({ costUSD: 0, savedCostUSD: 0.10 });
    const snap = ht.getHeadlineSnapshot();
    expect(snap.statsClause).toContain('$0.10 saved');
  ...
  it('omits multiplier when costUSD is non-finite', () => {
    const ht = new HeartbeatTimer(() => {}, { provider: 'gpt-5', parentModel: 'claude-opus-4-7' });
    ht.start(5);
    ht.applyCost({ costUSD: Infinity, savedCostUSD: 0.10 });
    const snap = ht.getHeadlineSnapshot();
    expect(snap.statsClause).toContain('$0.10 saved');
    expect(snap.statsClause).not.toContain('x');
  });
