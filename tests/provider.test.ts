...
// From the package root, not the `providers/provider-factory` subpath. That subpath was published
// in `exports`, which made `__setCoreTestProviderOverride`/`…Map` importable by any consumer — the
// exact hazard index.ts keeps them out of the barrel to prevent. `createProvider` is on the barrel,
// so nothing was lost by removing it.
import { createProvider, type MultiModelConfig } from '@zhixuan92/multi-model-agent-core';
