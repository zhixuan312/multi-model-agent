...
| `client` | string (1–120 chars, alphanumeric + `-_.:+/@`) | Client adoption tracking. The calling client's own name, drawn from the canonical roster (`claude-code`, `claude-desktop`, `codex`, `antigravity`, `cursor`, `vscode`, `opencode`, `windsurf`), or `mcp:<client>` when the task arrived over the MCP endpoint and the protocol carried the client's own name. Since 6.2.0 an MCP task whose protocol did not carry that name falls back to the identity the install declared in `X-MMA-Client` — including `agent-plugin`, which means the task arrived through an Agent Plugins package rather than a client-specific install — and only then to a bare `mcp`. Before 5.16.0 this was hardcoded `claude-code` regardless of caller. |
...
| `mainModel` | string (1–120 chars) or null | The flagship model the task was delegated FROM (e.g. `claude-opus-4-7`). Added in 3.12.2 alongside `mainModelFamily` so retrospective cost analysis doesn't have to assume the main agent identity. Since 6.6.0 it is read from the daemon's configured `agents.main.model`, which is a required tier, so records emitted by 6.6.0 and later always carry a value. The field stays nullable on the wire for records emitted by earlier versions, where a caller that sent no model claim produced null. |
...
| `mainCostUSD` | float (0–800) or null | What the SAME tokens would have cost on the main model. Since 6.6.0 it is null only when the configured `agents.main.model` has no pricing profile — an unrecognised main model yields no comparison rather than a guessed one. Added in 3.12.2 as `mainEquivalentCostUSD`; renamed to `mainCostUSD` in 4.7.6 to match the DB column `main_cost_usd`. |
...
`mainCostUSD = priceTokens(allTokens, parentRateCard)` — the same totals priced at the main model's rate. `costDeltaVsMainUSD = totalCostUSD − mainCostUSD`. Positive = paid more than parent would have. Negative = saved. Since 6.6.0 the parent rate card comes from the configured `agents.main.model`, so both are null only when that model has no pricing profile. Like cost, all of these are estimates, not guarantees.
