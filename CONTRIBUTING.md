...
- **Mock providers in tests.** Never call real LLM APIs from `tests/`. Use the `mockProvider` / `throwingProvider` / `capExhaustingProvider` helpers in `tests/contract/fixtures/mock-providers.ts`.
