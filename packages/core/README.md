...
- **Investigate retry path now produces findings.** Closes the 4.7.4 half-ship: investigate's brief-slot kept the legacy `## Summary / ## Citations / ## Confidence` format while only the read-route-implementer's `parallel-criteria-prompt` had been canonicalized to `## Finding N:` blocks. Direct `/investigate` calls worked, but `/retry` of an investigate task (routed through the standard implementer because retry's `toolCategory='assist'`) used the original prompt verbatim and emitted prose the parser dropped silently. Brief-slot now mandates canonical Findings; the standard-implementer path parses them too.
- **Findings-parser warnings now visible everywhere.** The `warnSink` callback that surfaces dropped Finding blocks (empty claim, missing severity, invalid evidence) had no consumers. Now wired through `envelope.validationWarnings` and projected onto the HTTP `/batch/:id` per-task result and the wire telemetry `validation_warnings` field — silent drops become observable on both operator-facing and analytics paths.
- **Claude file-path extraction.** `claude-session` extracts `file_path` (or `notebook_path`) from each `tool_use` block's input and records it through `envelope.recordToolCall`. Read counts toward `filesRead`; Write/Edit/MultiEdit toward `filesWritten`; NotebookEdit toward both. `Bash`/`Glob`/`Grep`/`WebFetch` record the call but no file activity — we only record what we can directly observe.
- **Adaptive polling-headline.** `/batch/:id` 202 body shows `tools=N` always and appends `reads=N`/`writes=N` only when each is > 0. The literal `reads=0 writes=0` no longer renders through the lifetime of every task.
- **From 4.7.4:** all findings-summary signals standardized at the top level of wire + HTTP records (per-stage `findingsBySeverity`/`findingsOutcome` removed); tolerant findings parser; `envelope.recordFinding` bridge wired.
