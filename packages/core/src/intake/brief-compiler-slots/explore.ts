...
export function exploreSlot(input: ExploreInput): ExploreBrief[] {
  const cwd = input.cwd ?? process.cwd();
  return [
    { taskIndex: 0, brief: `Explore '${input.topic}' from internal codebase`, cwd, agentType: 'complex' as const, reviewPolicy: 'none' as const, contextBlockIds: [], researchAdapter: 'internal' as const },
    { taskIndex: 1, brief: `Explore '${input.topic}' from external sources`, cwd, agentType: 'complex' as const, reviewPolicy: 'none' as const, contextBlockIds: [], researchAdapter: 'external' as const },
    { taskIndex: 2, brief: `Synthesize internal + external findings for '${input.topic}'`, cwd, agentType: 'complex' as const, reviewPolicy: 'none' as const, contextBlockIds: [], researchAdapter: 'synth' as const },
  ];
}
