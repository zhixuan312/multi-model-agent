// v4.4 — wrap mma's ToolImplementations in @openai/agents `tool()` factory.
// Ported from 3.12.7's `tools/openai-adapter.ts`. Consumed by
// `openai-agent-session.ts` to give the SDK a tool set without us having
// to provide a `Shell` impl (for `shellTool`) or an `Editor` impl (for
// `applyPatchTool`) — the SDK calls our closures directly.

import { tool } from '@openai/agents';
import { z } from 'zod';
import type { ToolImplementations } from './tool-impls.js';
import { READONLY_TOOL_IDS } from './tool-impls.js';

export type ToolMode = 'none' | 'full' | 'readonly' | 'no-shell';

export function createOpenAITools(impl: ToolImplementations, toolMode: ToolMode = 'full') {
