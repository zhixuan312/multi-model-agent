...
export function buildSystemPrompt(): string {
  return [
    ...
    '',
    'Tool selection — strictly enforce this priority. Reading these rules is mandatory before your first tool call:',
    '- File CONTENT inspection → read_file (whole file) or grep (search). NEVER run_shell with cat, head, tail, less, more, or sed -n.',
    '- File NAME / directory listing → glob or list_files. NEVER run_shell with find, ls, or fd.',
    '- File EDITING (any change to an existing file) → edit_file. NEVER run_shell with sed, awk, perl -i, tee, or > redirection.',
    '- File CREATION (new file or full overwrite) → write_file. NEVER run_shell with cat-heredoc or > redirection.',
    '- run_shell is ONLY for actions with side effects: running tests (npm test, vitest, pytest), running builds (npm run build, tsc, cargo build), running linters / type-checks, or git operations the brief explicitly asks for. If your goal is to read, search, list, or edit a file, do not call run_shell — use the dedicated tool above.',
    'Using run_shell for file inspection or editing is wrong every time — there is no exception. The harness does not track sed/awk/perl edits, so they fail review even when they appear to succeed.',
    '',
    'Tool rules:',
    '- Use grep to locate content. Use read_file when you need the full file context that grep cannot provide.',
