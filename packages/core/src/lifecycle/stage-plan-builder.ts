...
export function buildStagePlan(category: ToolCategory): StagePlan {
  ...
  const rows: StageRow[] = [
    ...
    // 4.5.x: settle_spec_chain — sets state.specChainPassed per spec § C10 "Prior chains passed"
    { rowId: '4.5.x', stageName: 'settle_spec_chain',
      runCondition: () => true, isRework: false, handlerKey: 'settle_spec_chain' },
    ...
    //      For read-only tools this is the AnnotatorEngine pass; verdict will be 'annotated', not 'concerns'.
    ...
    // 4.7: rework_for_quality_round_1 (caller-tier reset per C9) — only artifact-producing; annotator output 'annotated' naturally fails this gate
    { rowId: '4.7', stageName: 'rework_for_quality_round_1', schemaStage: 'quality_rework',
      runCondition: (s) => isAP && s.qualityReviewRound1Verdict === 'concerns' && !s.terminal,
    ...
    { rowId: '4.9', stageName: 'rework_for_quality_round_2', schemaStage: 'quality_rework',
      runCondition: (s) => isAP && s.qualityReviewRound2Verdict === 'concerns' && !s.terminal,
    ...
    // 4.10.x: settle_quality_chain — sets state.qualityChainPassed per spec § C10
    { rowId: '4.10.x', stageName: 'settle_quality_chain',
      runCondition: () => true, isRework: false, handlerKey: 'settle_quality_chain' },
    ...
    { rowId: '5.2', stageName: 'git_commit', schemaStage: 'committing',
      runCondition: (s) => (s as any).autoCommit === true && Array.isArray((s as any).filesChanged) && (s as any).filesChanged.length > 0 && !(s as any).readOnlyTask && !s.terminal,
      isRework: false, handlerKey: 'git_commit' },
    // 5.3: compose_response — always fires
    { rowId: '5.3', stageName: 'compose_response',
      runCondition: () => true, isRework: false, handlerKey: 'compose_response' },
    // 5.3.5: register_terminal_block — every task except register_context_block (per spec line 1342)
    { rowId: '5.3.5', stageName: 'register_terminal_block',
      runCondition: (s) => s.route !== 'register-context-block', isRework: false, handlerKey: 'register_terminal_block' },
