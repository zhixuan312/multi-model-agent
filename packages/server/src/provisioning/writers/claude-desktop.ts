 ...
 *  never a credential.
 *
 *  `--client=claude-desktop` is what makes Desktop attribute as itself; without it
 *  the bridge sends no `X-MMA-Client` and the daemon records the anonymous `mcp`.
 *  `isOwnedMcpEntry` accepts a closed set of two arg shapes so this stays
 *  recognisable alongside entries written before the flag existed. */
function buildEntry(input: WriteClientRegistrationInput): { command: string; args: string[] } {
  return {
    command: input.execPath ?? process.execPath,
    args: [input.cliEntrypoint, 'mcp', '--client=claude-desktop'],
  };
}
