...
5. **Q5 — progressTrace shape**: per-task trace (not per-batch), bounded by both count AND size (80 events / 16 KB), priority-based trimming with `text_emission` and `tool_call` dropped first, opt-in via `includeProgressTrace`.
