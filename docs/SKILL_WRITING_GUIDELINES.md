...
```markdown
**Use `target.paths` to anchor narrow questions:**
...
Split heavy reference (large request schemas, terminal-envelope shapes) into separate `_shared/*.md` files inside the skill directory. Reference them with `@include _shared/<file>.md`.
...
| **Medium freedom** (preferred pattern) | Pseudocode + parameters | "`reviewPolicy: 'none'` for mechanical refactors; default `'reviewed'` otherwise" |
| **Low freedom** (fragile sequence) | Exact tool-call shape, no flexibility | "Call `mma_run` with exactly `{ cwd, request: { type: 'audit', subtype: 'plan', target: { paths: [planPath] } } }`" |
...
❌ See @packages/server/src/skills/mma-context-blocks/SKILL.md for the context-block flow.
```
...
JSON example + field table (the object that rides inside `request`).

## Full example

The exact `mma_run` tool-call JSON, copy-pasteable.
...
`tests/contract/skills/skill-frontmatter.test.ts`, `tests/skills/skill-validity.test.ts`, and
`tests/contract/skills/mcp-only-client-surface.test.ts` check:
