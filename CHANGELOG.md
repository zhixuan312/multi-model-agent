...
- **Separation and invariants (Items 11, 12, 13, Wave 20).** Added `forbiddenModels`/`forbiddenTiers` to escalation orchestrator so reviewer-implementer separation gate is enforced under fallback. Tier-based separation (standard/complex/main) supersedes the initial model-name comparison — the user's model choice is sovereign; tier is the mechanism gate. Fixed R4 stage-duration sum invariant to use wall-clock truth with defensive clamping. Attached `validation_warnings` to emitted events so schema refinement violations are reported, not silently dropped.
- **Runner layer (Items 10, 14, 4).** Normalized token semantics across providers (R6a): all runners now emit `inputTokens`/`outputTokens` with consistent definitions. Cost calculation consumes `cachedTokens` (R6b) so per-stage cost breakdowns reflect actual billed tokens. Split reviewer prompts into `systemPrefix` + `userBody` for cross-runner caching — stable prefix gets `cache_control: ephemeral` markers on Claude/Codex runners.
- **Intake plumbing (Items 6, 19).** Threaded `verifyCommand` from HTTP brief through `TaskSpec` → lifecycle → telemetry, closing a four-point gap that silently dropped the field. Threaded `reviewPolicy` and `verifyCommandPresent` through the recorder callback so emitted events carry the caller's actual values, not route-based defaults.
